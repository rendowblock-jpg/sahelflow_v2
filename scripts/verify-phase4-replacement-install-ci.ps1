...
try {
    ...
    # The committed restore can restart WebView2 on the fixed evidence-only
    # debugging port. Recent hosted Windows runners have twice exposed a page
    # target through /json/list before its CDP WebSocket is connectable. Keep
    # the production fail-closed mutation rule intact: first prove the CDP
    # transport with a read-only expression. If that exact post-restore target
    # is stale, recycle the installed process once before any owner/trial
    # mutation, then require the read-only preflight to succeed.
    $acceptanceFunctionAnchor = 'function Invoke-CommittedWebViewAcceptance {'
    $acceptanceFunctionFirst = $source.IndexOf($acceptanceFunctionAnchor, [StringComparison]::Ordinal)
    $acceptanceFunctionLast = $source.LastIndexOf($acceptanceFunctionAnchor, [StringComparison]::Ordinal)
    if ($acceptanceFunctionFirst -lt 0 -or $acceptanceFunctionFirst -ne $acceptanceFunctionLast) {
        throw "Replacement harness committed-WebView acceptance function anchor drifted."
    }
    $transportPreflightFunction = @'
function Test-CommittedWebViewTransport {
    param(
    ...
    do {
        try {
            ...
            foreach ($target in $candidates) {
                try {
                    $readyState = Invoke-WebViewExpressionForTarget `
                        -WebSocketUrl ([string]$target.webSocketDebuggerUrl) `
                        -Expression "document.readyState"
                    if ([string]$readyState -in @("loading", "interactive", "complete")) {
                        return $true
                    }
                } catch {
                    # This probe is read-only. A stale post-restore page target
                    # may be retried or recycled before any owner/trial mutation.
                }
            }
        } catch {
            # The fixed CI-only debugging endpoint appears asynchronously.
        }
...
'@
    $source = $source.Replace(
        $acceptanceFunctionAnchor,
        "$($transportPreflightFunction.TrimEnd())`r`n`r`n$acceptanceFunctionAnchor"
    )

    $committedStartAnchor = @'
...
'@
    $committedStartFirst = $source.IndexOf($committedStartAnchor.TrimEnd(), [StringComparison]::Ordinal)
    $committedStartLast = $source.LastIndexOf($committedStartAnchor.TrimEnd(), [StringComparison]::Ordinal)
    if ($committedStartFirst -lt 0 -or $committedStartFirst -ne $committedStartLast) {
        throw "Replacement harness committed-restore startup anchor drifted."
    }
    $committedStartReplacement = @'
$committedProcess = Start-SahelFlow
$committedEndpoint = Wait-ForRuntime $committedProcess "committed replacement restore"
$committedBaseUrl = [string]$committedEndpoint.appUrl
if (-not (Test-CommittedWebViewTransport -BaseUrl $committedBaseUrl -TimeoutSeconds 15)) {
    # The restore has already committed and no owner/trial mutation has been
    # dispatched. Recycle only this ephemeral installed process once so the
    # fixed debugging port exposes a fresh, connectable page target.
    Stop-ResidualSahelFlow
    $committedProcess.Dispose()
    $committedProcess = Start-SahelFlow
    $committedEndpoint = Wait-ForRuntime $committedProcess "committed replacement restore transport recycle"
    $committedBaseUrl = [string]$committedEndpoint.appUrl
    if (-not (Test-CommittedWebViewTransport -BaseUrl $committedBaseUrl -TimeoutSeconds 15)) {
        throw "The committed replacement WebView CDP transport did not become connectable before owner/trial mutation."
    }
}
$restoredBeforeReenrollment = Get-ProfileEvidence
'@
    $source = $source.Replace(
        $committedStartAnchor.TrimEnd(),
        $committedStartReplacement.TrimEnd()
    )
