...
param(
    ...
    [string]$ExpectedDisplayVersion = '1.0.0.19',
    [string]$ExpectedAppVersion = '1.0.0-internal.19'
)
...
function Get-TrackedDataState {
    ...
    if (Test-Path -LiteralPath $registryPath -PathType Leaf) { $candidates.Add($registryPath) }
    $shopsPath = Join-Path $RoamingData 'shops'
    if (Test-Path -LiteralPath $shopsPath -PathType Container) {
        Get-ChildItem -LiteralPath $shopsPath -File -Filter '*.db' -ErrorAction Stop | ForEach-Object { $candidates.Add($_.FullName) }
    }
...
if ($SelfTest) {
    if ([string]::IsNullOrWhiteSpace($ScriptRoot)) { throw 'Windows PowerShell could not resolve the script directory.' }
    if ($ExpectedDisplayVersion -notmatch '^\d+\.\d+\.\d+\.\d+$') { throw 'ExpectedDisplayVersion is not a four-part MSI version.' }
    if ($ExpectedAppVersion -notmatch '^\d+\.\d+\.\d+-internal\.\d+$') { throw 'ExpectedAppVersion is not an internal SahelFlow version.' }
    Write-Host "Founder installer self-test passed under Windows PowerShell $($PSVersionTable.PSVersion)." -ForegroundColor Green
    exit 0
}

if ([string]::IsNullOrWhiteSpace($MsiPath)) { throw 'MsiPath is required. Pass the exact signed MSI path explicitly.' }
if ($ExpectedMsiSha256 -notmatch '^[0-9A-Fa-f]{64}$') { throw 'ExpectedMsiSha256 must be the exact 64-character SHA-256 supplied with the handoff.' }
...
$running = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
    ...
    -not [string]::IsNullOrWhiteSpace($path) -and $path.StartsWith("$InstallDirectory\", [System.StringComparison]::OrdinalIgnoreCase)
})
if ($running.Count -ne 0) { throw 'SahelFlow or one of its runtime children is running. Close SahelFlow normally before installing.' }

$resolvedMsiPath = (Resolve-Path -LiteralPath $MsiPath).Path
$actualHash = (Get-FileHash -LiteralPath $resolvedMsiPath -Algorithm SHA256).Hash
if ($actualHash -ine $ExpectedMsiSha256) { throw "MSI SHA-256 mismatch. Expected $ExpectedMsiSha256, found $actualHash. Installation blocked." }
...
if (-not $dataPreserved) { throw 'Tracked shop registry/database state changed during installation. Launch blocked.' }

$installed = Get-InstalledSahelFlow
if ($null -eq $installed) { throw 'SahelFlow is not registered after installation.' }
if ($installed.DisplayVersion -ne $ExpectedDisplayVersion) { throw "Installed version mismatch. Expected $ExpectedDisplayVersion, found $($installed.DisplayVersion)." }
if (-not (Test-Path -LiteralPath $ExecutablePath -PathType Leaf)) { throw "Installed executable not found: $ExecutablePath" }
$executableVersion = (Get-Item -LiteralPath $ExecutablePath).VersionInfo.ProductVersion
if ($executableVersion -ne $ExpectedAppVersion) { throw "Installed executable version mismatch. Expected $ExpectedAppVersion, found $executableVersion." }
...
$endpointCandidates = @((Join-Path $RoamingData 'runtime-endpoint.json'), (Join-Path $LocalData 'runtime-endpoint.json'))
$uiReadyCandidates = @((Join-Path $RoamingData 'runtime-ui-ready.json'), (Join-Path $LocalData 'runtime-ui-ready.json'))
$diagnosticCandidates = @((Join-Path $RoamingData 'startup-diagnostic.json'), (Join-Path $LocalData 'startup-diagnostic.json'))
...
while ((Get-Date) -lt $deadline) {
    ...
    $app.Refresh()
    if ($app.HasExited) { throw "SahelFlow exited before authenticated UI readiness with code $($app.ExitCode)." }
    ...
    foreach ($candidate in $endpointCandidates) {
        ...
        if ($null -ne $parsed -and $parsed.state -eq 'ready' -and $parsed.appVersion -eq $ExpectedAppVersion -and [int64]$parsed.processId -eq [int64]$app.Id -and -not [string]::IsNullOrWhiteSpace([string]$parsed.instanceId)) {
            $endpoint = $parsed
    ...
    foreach ($candidate in $uiReadyCandidates) {
        ...
        if ($null -eq $item -or $item.LastWriteTime -lt $launchStartedAt.AddSeconds(-2)) { continue }
        $parsed = Read-JsonFile -Path $candidate
        if ($null -ne $parsed -and [int]$parsed.formatVersion -eq 1 -and [int]$parsed.protocolVersion -eq $ExpectedProtocolVersion -and $parsed.state -eq 'ready' -and $parsed.appVersion -eq $ExpectedAppVersion -and $parsed.pageUrl -match '^http://(127\.0\.0\.1|localhost):\d+$') {
            $uiReady = $parsed
    ...
    $app.Refresh()
    if ($null -ne $endpoint -and $null -ne $uiReady -and $uiReady.instanceId -eq $endpoint.instanceId -and $app.MainWindowHandle -ne 0 -and $app.Responding) { break }
}

$app.Refresh()
if ($null -eq $endpoint -or $null -eq $uiReady -or $uiReady.instanceId -ne $endpoint.instanceId -or $app.MainWindowHandle -eq 0 -or -not $app.Responding) {
