...
const requiredRoutes = [
  "/", "/login", "/setup", "/join", "/dashboard", "/orders",
  "/orders/confirmation-queue", "/customers", "/products", "/deliveries",
  "/returns", "/accounting", "/accounting/cod-reconciliation", "/analytics",
  "/risk", "/imports", "/inbox", "/automations", "/agents", "/storefronts",
  "/settings", "/profile",
] as const;
...
for (const primitive of ["EntityLink", "EntityPreview", "EntityInspector", "EntityTimeline"]) {
  if (!entityContext.includes(`function ${primitive}`)) errors.push(`entity context: ${primitive} primitive is missing`);
}
...
if (!customerTable.includes("EntityInspector") || !customerTable.includes("EntityLink")) errors.push("customers: shared entity link/inspector contract is not adopted");
if (!productTable.includes("EntityInspector") || !productTable.includes("EntityLink")) errors.push("products: shared entity link/inspector contract is not adopted");
if (!deliveryDetail.includes("EntityTimeline") || !deliveryDetail.includes("EntityLink")) errors.push("deliveries: shared entity link/timeline contract is not adopted");

const chartFrame = source("src/components/charts/chart-primitives.tsx");
if (!chartFrame.includes("accessibleSummary") || !chartFrame.includes("aria-describedby={summaryId}")) errors.push("charts: governed textual summary contract is missing");
const stateSurface = source("src/components/shared/state-surface.tsx");
if (!stateSurface.includes("data-state-tone") || !stateSurface.includes("aria-live")) errors.push("state surface: shared persistent semantic state contract is incomplete");
const dataTable = source("src/components/data-table/data-table.tsx");
if (dataTable.includes("tabIndex={onRowClick") || dataTable.includes('event.key === "Enter"')) errors.push("data table: semantic rows must not be custom keyboard controls");
...
for (const path of [
  ...
  if (!content.includes("pageSize") || !content.includes("hasNextPage")) errors.push(`${path}: server-authoritative pagination contract is incomplete`);
}

for (const path of ["src/app/login/page.tsx", "src/app/setup/page.tsx", "src/app/join/page.tsx"]) {
  const content = source(path);
  for (const legacy of ["bg-gradient-to", "animate-scale-in", "shadow-popover"]) {
    if (content.includes(legacy)) errors.push(`${path}: legacy decorative entry-surface token remains (${legacy})`);
  }
}

const exportFoundation = source("src/lib/import/paged-export.ts");
if (!exportFoundation.includes("createPagedCsvStream") || !exportFoundation.includes("collectBoundedXlsxRows")) {
  errors.push("exports: paged CSV / bounded XLSX shared contract is missing");
}
for (const path of [
  ...
  const content = source(path);
  if (content.includes("take: 10000")) errors.push(`${path}: hidden 10,000-row export cap remains`);
  if (!content.includes("createPagedCsvStream") || !content.includes("collectBoundedXlsxRows")) {
    errors.push(`${path}: complete export does not use the paged CSV / bounded XLSX contract`);
  }
}

const importsPage = source("src/app/(dashboard)/imports/page.tsx");
if (!importsPage.includes('id="import-orders"') || !importsPage.includes('entity="orders"')) {
  errors.push("imports: governed order preview/commit workbench is missing");
}
...
const rows = routeMatrix.map((entry) =>
