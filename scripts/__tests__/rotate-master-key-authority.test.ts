...
describe("installation-wide master-key rotation authority", () => {
  ...
  it("uses one old/new key pair for every target before committing the keyfile", () => {
    const targetLoop = source.indexOf("for (const target of targets)", source.indexOf("const allStats"));
    const rotateTarget = source.indexOf("rotateTarget(target, oldKey, newKey)", targetLoop);
    ...
    expect(source.slice(rotateTarget, commitKeyfile)).not.toContain("renameSync(SIDECAR_PATH");
  });

  it("re-wraps Secrets for every shop and fails instead of skipping corruption", () => {
    expect(source).toContain("await rotateSecrets(client, target.id, oldKey, newKey)");
    expect(source).toContain("is undecryptable");
    expect(source).not.toContain("Multi-shop rotation. The script operates on the DATABASE_URL shop only");
    expect(source).not.toContain("run once per shop");
  });

  it("keeps the old keyfile authoritative until every database succeeds", () => {
    expect(source).toContain("A crash leaves the old keyfile plus reusable sidecar");
    expect(source).toContain("loadOrCreateNewKey(oldKey)");
    expect(source).toContain("Keep ${SIDECAR_PATH}; rerunning will resume with the same new key.");
  });
});
