  ...
  return (
    <div className="app-content page-sections">
      ...
      <div className="card-grid-3">
        <StatCard label={t("automations.total")} value={automations.length} icon={<Bot />} />
        <StatCard label={t("common.active")} value={activeCount} icon={<Zap />} />
        <StatCard label={t("automations.totalRuns")} value={totalRuns} icon={<CheckCircle2 />} />
      </div>

      {automations.length > 0 ? (
        <Card>
          <CardHeader><CardTitle className="text-base">{t("automations.yourAutomations")}</CardTitle></CardHeader>
                ...
                <div key={automation.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                      <span>{t("automations.triggerLabel")}: {t(TRIGGER_I18N[automation.trigger] ?? automation.trigger)}</span>
                      <span>·</span>
                      <span>{t("automations.runsLabel")}: {automation.runCount}</span>
                      {automation.lastRunAt ? (
                        <><span>·</span><span className="flex items-center gap-1"><Clock className="size-3" aria-hidden="true" />{formatDate(automation.lastRunAt, locale)}</span></>
                      ) : null}
                    </div>
                  </div>
                  {canManage ? (
                    <div className="flex items-center gap-1">
                      ...
                      />
                      <AutomationActions variant="toggle" automationId={automation.id} isActive={automation.isActive} />
                    </div>
                  ) : null}
                </div>
              ))}
      ...
      {canManage ? <AutomationRunRecoveryPanel initialRuns={recentRuns} /> : null}

      {canManage ? (
        <Card>
          <CardHeader><CardTitle className="text-base">{t("automations.templates")}</CardTitle></CardHeader>
          <CardContent className="p-0">
            <div className="divide-y">
              {recipes.map((recipe) => (
                <div key={recipe.nameKey} className="flex items-start justify-between gap-3 p-4">
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      <Badge variant="outline">{t(TRIGGER_I18N[recipe.trigger] ?? recipe.trigger)}</Badge>
                      <span className="icon-rtl-flip text-muted-foreground">→</span>
                      <Badge variant="outline">{t(ACTION_I18N[recipe.action] ?? recipe.action)}</Badge>
                    </div>
                  </div>
                  <AutomationActions variant="activate" recipeName={t(recipe.nameKey)} trigger={recipe.trigger} action={recipe.action} />
                </div>
              ))}
            </div>
          </CardContent>
      ...
      {recentLogs.length > 0 ? (
        <Card>
          <CardHeader><CardTitle className="text-base">{t("automations.recentActivity")}</CardTitle></CardHeader>
          <CardContent className="p-0">
              ...
              {recentLogs.map((log) => (
                <div key={log.id} className="flex items-start justify-between gap-3 p-3 text-sm">
                  <div className="min-w-0 space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className={`size-1.5 shrink-0 rounded-full ${log.status === "success" ? "bg-success" : log.status === "failed" ? "bg-destructive" : "bg-warning"}`} />
                      <span className="truncate font-medium">{log.automation.name}</span>
                    </div>
                    {log.message ? <p className="truncate ps-3.5 text-xs text-muted-foreground">{log.message}</p> : null}
                  </div>
                  <span className="shrink-0 whitespace-nowrap text-xs text-muted-foreground">{formatDate(log.createdAt, locale)}</span>
                </div>
              ))}
            </div>
          </CardContent>
