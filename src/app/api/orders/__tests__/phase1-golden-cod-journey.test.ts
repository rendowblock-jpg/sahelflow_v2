...
beforeEach(async () => {
  ...
  harness.getCurrentSessionAuthority.mockReset().mockResolvedValue({
...
describe("Phase 1 representative Golden COD journey", () => {
  it("passes storefront UI/API/domain/database authority through delivery, COD, return, refund and reversal", async () => {
    const base = Date.now();
    ...
    const createdResponse = await submitStorefront(
      mockPost("http://localhost/api/storefront/submit", storefrontPayload),
    );
    ...
    const decisionResponse = await decideOrder(
      mockPost(`http://localhost/api/orders/${created.orderId}/decision`, {
        decision: "confirm",
        expectedVersion: 1,
    ...
    async function fulfill(
      ...
      const response = await fulfillOrder(
        mockPost(`http://localhost/api/orders/${created.orderId}/fulfillment`, {
          action,
          expectedVersion,
    ...
    const returnPositionResponse = await getCustomerReturn(
      mockGet(`http://localhost/api/orders/${created.orderId}/customer-return`),
      orderParams(created.orderId),
    );
    ...
    async function transition(
      action:
        "approve" | "mark_in_transit" | "receive" | "inspect" | "complete",
      minute: number,
      items?: Array<{
        orderItemId: string;
        quantity: number;
      ...
      const response = await transitionCustomerReturn(
        ...
        returnParams(created.orderId, requestedReturn.returnCase.returnId),
      );
      const body = await responseJson<{
    ...
    const refundResponse = await issueRefund(
      mockPost(`http://localhost/api/orders/${created.orderId}/refunds`, {
    ...
    expect(harness.dispatchTrigger).toHaveBeenCalledTimes(3);
  }, 180_000);
