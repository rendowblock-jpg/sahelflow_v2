...
export interface SettingsSurfaceAccess { settingsManage: boolean; sessionsManage: boolean; membersManage: boolean; licenseManage: boolean; integrationsManage: boolean; deliveryCredentialsManage: boolean; backupManage: boolean; riskManage: boolean; dangerManage: boolean; }
const TABS: Array<{ id: Tab; icon: typeof Shield; labelKey: string; allowed: (access: SettingsSurfaceAccess) => boolean }> = [
  ...
  { id: "security", icon: ShieldCheck, labelKey: "settings.tabs.security", allowed: (a) => a.sessionsManage },
  { id: "team", icon: Users, labelKey: "settings.tabs.team", allowed: (a) => a.membersManage },
  { id: "appearance", icon: Palette, labelKey: "settings.tabs.appearance", allowed: () => true },
  { id: "license", icon: Shield, labelKey: "settings.tabs.license", allowed: (a) => a.licenseManage },
  ...
  { id: "integrations", icon: Store, labelKey: "settings.tabs.integrations", allowed: (a) => a.integrationsManage },
  { id: "phone", icon: Phone, labelKey: "settings.tabs.phoneReputation", allowed: (a) => a.riskManage },
  { id: "backup", icon: DatabaseBackup, labelKey: "settings.tabs.backup", allowed: (a) => a.backupManage },
  { id: "danger", icon: AlertTriangle, labelKey: "settings.tabs.dangerZone", allowed: (a) => a.dangerManage },
];
...
export function SettingsTabs({ access, integrations }: { access: SettingsSurfaceAccess; integrations: Array<{ platform: string; status: string }> }) {
  ...
  const select = (index: number) => { const normalized = (index + visibleTabs.length) % visibleTabs.length; const tab = visibleTabs[normalized]; if (!tab) return; void setQueryTab(tab.id); queueMicrotask(() => refs.current[normalized]?.focus()); };
