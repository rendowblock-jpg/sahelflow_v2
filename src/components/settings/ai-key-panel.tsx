...
export function AiKeyPanel({
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      const requests: Promise<unknown>[] = [loadConsent()];
      if (canManageKey) requests.push(loadKeyStatus());
      void Promise.all(requests);
    }, 0);
    return () => window.clearTimeout(timeoutId);
  }, [canManageKey, loadConsent, loadKeyStatus]);

  const verifyPin = async () => {
    if (!canManageKey || !pin.trim() || reauthBusy) return;
    setReauthBusy(true);
    setReauthError(null);
    try {
              ...
              {consentState === "loading" ? (
                ...
                <div className="space-y-2">
                  <label
                    ...
                    className="flex select-none items-start gap-2.5 text-sm font-medium"
                  >
                    <Checkbox
                      id="gemini-consent"
                      ...
                      disabled={consentSaving || !canManageConsent}
                      className="mt-0.5"
                    />
