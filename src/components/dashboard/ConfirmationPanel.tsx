...
import {
  Phone, MessageCircle, AlertTriangle, CheckCircle,
  XCircle, Voicemail, Copy, ChevronRight, TrendingUp, Plus,
} from 'lucide-react'
...
interface Props {
  order: OrderData
  onStatusChange: (orderId: string, newStatus: string, extra?: Record<string, unknown>) => void
  onClose: () => void
}
...
export default function ConfirmationPanel({ order, onStatusChange, onClose: _onClose }: Props) {
  const { t } = useI18n()
  ...
  const [upsellSuggestions, setUpsellSuggestions] = useState<Array<{ product_id: string; name: string; price: number; margin: number; marginPercent: number; stock: number; reason: string }>>([])
  ...
  function toggleStep(index: number) {
    ...
    setStepChecked(next)
    if (index === 6 && next.has(6) && upsellSuggestions.length === 0 && !upsellLoading) {
  ...
  async function fetchUpsellSuggestions() {
    ...
    try {
      ...
      toast({ type: 'error', title: t.confirmation?.upsellFailed || 'Failed to load upsell suggestions' })
    } finally {
  ...
  async function markConfirmationStatus(status: ConfirmationStatus) {
    ...
    try {
      ...
      const newAttempts = status === 'rappel' || status === 'boite_vocale' ? attempts + 1 : attempts
      ...
      toast({ type: 'error', title: t.confirmation?.statusUpdateFailed || 'Failed to update status' })
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleConfirmOrder() {
    ...
    try {
      ...
      await supabase
        ...
        .eq('id', order.id)
      onStatusChange(order.id, 'confirmed', { confirmation_status: 'confirmed' })
    } catch {
      toast({ type: 'error', title: t.confirmation?.confirmFailed || 'Failed to confirm order' })
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleReturnWithReason() {
    ...
    try {
      ...
      await supabase
        .from('orders')
        .update({ return_reason: returnReason })
        .eq('id', order.id)
      onStatusChange(order.id, 'returned', { return_reason: returnReason })
    } catch {
      toast({ type: 'error', title: t.confirmation?.returnFailed || 'Failed to process return' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 4 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ fontSize: 18, fontWeight: 700 }}>{t.confirmationFlow.title}</h2>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {confirmationStatus && (
            <span style={{
              ...
              {t.confirmationStatuses[confirmationStatus as keyof typeof t.confirmationStatuses] || confirmationStatus}
            </span>
          )}
          {attempts > 0 && (
            <span style={{ fontSize: 11, color: 'var(--color-content-tertiary)' }}>
      ...
      <div style={{
        background: 'var(--color-surface-primary)', border: '1px solid var(--color-line-primary)',
        ...
        <div>
          ...
          {phone && <p style={{ fontSize: 13, color: 'var(--color-content-secondary)', fontFamily: 'monospace' }}>{phone}</p>}
          {order.wilaya && <p style={{ fontSize: 12, color: 'var(--color-content-tertiary)' }}>{order.wilaya}{order.commune ? `, ${order.commune}` : ''}</p>}
        </div>
          ...
          {phone && (
            <>
              <a href={`tel:${phone}`} style={{
                ...
                borderRadius: 8, background: '#10b981', color: '#fff', textDecoration: 'none',
      ...
      <div style={{
        ...
        <p style={{ fontWeight: 600, fontSize: 13, marginBottom: 8, fontFamily: 'monospace' }}>{order.order_number}</p>
        {(order.items || []).map((item, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '3px 0' }}>
            <span>{item.quantity}x {item.product_name}</span>
            <span style={{ fontFamily: 'monospace' }}>{item.unit_price.toLocaleString('fr-DZ')} DA</span>
          </div>
        ))}
        <div style={{ borderTop: '1px solid var(--color-line-primary)', marginTop: 6, paddingTop: 6, display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: 14 }}>
          <span>{t.store.total}</span>
          <span style={{ color: 'var(--color-brand-400)' }}>{Number(order.total_price).toLocaleString('fr-DZ')} DA</span>
        </div>
      </div>

      {/* 8-Step Confirmation Script */}
      <div>
        <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>{t.confirmationFlow.callScript}</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {CONFIRMATION_STEPS.map((stepKey, i) => (
            <label key={i} style={{
              display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px',
              borderRadius: 8, cursor: 'pointer', fontSize: 13,
              background: stepChecked.has(i) ? 'rgba(16,185,129,0.06)' : 'transparent',
      ...
      {stepChecked.has(6) && (
        <div>
          <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
            <TrendingUp size={16} style={{ color: '#f59e0b' }} /> {t.confirmationFlow.upsellPrompt}
          </h3>
          {upsellLoading ? (
            <p style={{ fontSize: 13, color: 'var(--color-content-tertiary)' }}>{t.common.loading}</p>
          ) : upsellSuggestions.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--color-content-tertiary)' }}>No upsell suggestions available</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {upsellSuggestions.map((s) => (
                <div key={s.product_id} style={{
                  ...
                  border: '1px solid var(--color-line-primary)', background: 'var(--color-surface-primary)',
                }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontWeight: 600, fontSize: 13 }}>{s.name}</p>
                    <p style={{ fontSize: 11, color: 'var(--color-content-tertiary)' }}>{s.reason}</p>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ fontWeight: 700, fontSize: 13 }}>{s.price.toLocaleString('fr-DZ')} DA</p>
                    <p style={{ fontSize: 11, color: '#10b981' }}>{s.marginPercent}% margin</p>
                  </div>
                </div>
              ))}
              {!upsellOffered && (
                <button
                  ...
                  onClick={async () => {
                    const supabase = createClient()
                    await supabase.from('orders').update({ upsell_offered: true }).eq('id', order.id)
      ...
      {/* Confirmation Notes */}
      <div>
        <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-content-secondary)', marginBottom: 4, display: 'block' }}>Notes</label>
        <textarea
          value={notes}
      ...
      <div>
        <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>Status</h3>
      ...
      {(order.status === 'returned' || order.status === 'refused') && !showReturnReason && (
        <button
          className="sf-btn sf-btn-ghost"
      ...
      {showReturnReason && (
        <div style={{
          ...
          <p style={{ fontWeight: 600, fontSize: 13, marginBottom: 8 }}>{t.returnReasons.selectReason}</p>
          <select
            value={returnReason}
...
function actionBtnStyle(color: string): React.CSSProperties {
  return {
    ...
    color, fontWeight: 600, fontSize: 12, cursor: 'pointer', transition: 'all 0.15s',
  }
}
