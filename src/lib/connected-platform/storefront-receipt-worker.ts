...
const CURSOR_KEY = "connected.storefront.receipt.cursor.v1";
...
export function startStorefrontReceiptWorker(): void {
  ...
  const tick = async () => {
    ...
    try {
      ...
      const stored = await db.setting.findUnique({ where: { key: CURSOR_KEY } });
      ...
      if (result.nextCursor > after) {
        await db.setting.upsert({
          ...
          create: { key: CURSOR_KEY, value: String(result.nextCursor) },
