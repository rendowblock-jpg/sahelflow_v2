...
describe("installed Windows runtime contract", () => {
  ...
  it("builds and launches the installed executable twice on an ephemeral Windows runner", () => {
    ...
    expect(harness).toContain("Wait-ForPromptVisibleWindow");
    expect(harness).toContain("RUNTIME_UI_READY_PERSISTED");
    expect(harness).toContain("startup-trace-launch-$attempt.json");
