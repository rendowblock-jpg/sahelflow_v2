...
/**
 * Acquire one command key for this browser-scoped draft generation.
 *
 * The random persisted scope prevents an identical order created from another
 * browser profile/device from replaying an earlier seller command. Tabs sharing
 * the same restored browser draft still converge on the same scope, request
 * fingerprint and generation. A successful response advances only that request
 * generation; an in-flight tab retains its original key for lost-response replay.
 */
export function resolveManualOrderCommand(
  ...
  const deterministicKey = deterministicCommandKey(
    scopeId,
    fingerprint,
    generation,
  );
