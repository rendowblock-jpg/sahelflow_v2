...
import { buildFrozenAutomationOutboxIntents } from "@/lib/business-truth/automation-outbox";
import {
...
async function reserveOrderItem(
  ...
  if (item.productVariantId) {
    ...
    // A null variant id + null snapshot name is the immutable parent-stock
    // authority chosen at trusted intake. Later catalog variants do not rewrite
    // that already-submitted order's stock target.
  ...
  return executeBusinessCommand(
    ...
    async ({ tx, commandId, aggregateVersion, principal }) => {
      ...
      const lowStockProducts =
        data.decision === "confirm"
          ? await findLowStockProducts(tx, reservations)
          : [];
      ...
      const trigger =
        data.decision === "confirm" ? "order.confirmed" : "order.cancelled";
      ...
      const decisionOutbox = await buildFrozenAutomationOutboxIntents(
        tx,
        commandId,
        trigger,
        {
      ...
      const lowStockOutboxGroups = await Promise.all(
        lowStockProducts.map((product) =>
          buildFrozenAutomationOutboxIntents(
            tx,
            commandId,
            "stock.low",
            {
      ...
      return {
        ...
        audit: {
          ...
          after: {
            ...
            lowStockEffectCount: lowStockProducts.length,
          },
          metadata: {
            decision: data.decision,
            authority: "manual-confirmation-v1",
