...
export function resolveProductWorkbenchAccess(
  ...
  return Object.freeze({
    ...
    import:
      allowed(actorContext, "data.import") && manage && cost && costUpdate,
  });
}
...
function orderByFor(sort: ProductWorkbenchSort): Prisma.ProductOrderByWithRelationInput[] {
  const [field, direction] = sort.split(".") as [
    "createdAt" | "price" | "stock",
    "asc" | "desc",
  ];
  return [{ [field]: direction }, { id: direction }] as Prisma.ProductOrderByWithRelationInput[];
}
...
export async function getProductsWorkbenchPage(
  ...
  const offset = query.offset != null ? Math.max(0, query.offset) : (page - 1) * pageSize;
