...
export async function retryWhatsAppInbound(
  ...
  await context.prisma.$transaction(async (tx) => {
    ...
    await tx.providerIngressEvent.update({
      ...
      data: {
        status: "received",
        // Manual recovery starts a new bounded attempt budget. Prior attempts
        // remain immutable and visible in ProviderIngressAttempt history.
        attemptCount: 0,
        nextAttemptAt: null,
        lockedAt: null,
        leaseToken: null,
    ...
    await tx.auditLog.create({
      data: {
        ...
        after: JSON.stringify({ status: "received", attemptCount: 0 }),
