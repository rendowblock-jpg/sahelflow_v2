...
const storefrontSectionSchema = z
  .object({
    ...
    settings,
    blocks: z
      .array(z.object({ id, type: id, settings }).strict())
      .max(50),
  })
  .strict()
  .superRefine((section, context) => {
    if (section.type === "media") {
      addMaxTextIssue(
        context,
        settingText(section.settings, "eyebrow"),
        80,
        ["settings", "eyebrow"],
        "Media eyebrow",
      );
      addMaxTextIssue(
        context,
        settingText(section.settings, "title"),
        140,
        ["settings", "title"],
        "Media title",
      );
      addMaxTextIssue(
        context,
        settingText(section.settings, "body"),
        1_000,
        ["settings", "body"],
        "Media body",
      );
      addMaxTextIssue(
        context,
        settingText(section.settings, "imageAlt"),
        240,
        ["settings", "imageAlt"],
        "Media image description",
      );

      const imageUrl = settingText(section.settings, "imageUrl");
      if (imageUrl && imageUrl.trim()) {
        const parsed = httpsUrl.safeParse(imageUrl.trim());
        if (!parsed.success) {
          context.addIssue({
            code: "custom",
            path: ["settings", "imageUrl"],
            message: "Media image URL must use HTTPS",
          });
        }
      }

      const align = settingText(section.settings, "align");
      if (align && align !== "split" && align !== "media-end") {
        context.addIssue({
          code: "custom",
          path: ["settings", "align"],
          message: "Unsupported media alignment",
        });
      }
    }
    ...
    if (section.type === "footer") {
      addMaxTextIssue(
        context,
        settingText(section.settings, "tagline"),
        240,
        ["settings", "tagline"],
        "Footer tagline",
      );
      ...
      section.blocks.forEach((block, index) => {
        if (block.type !== "testimonial") {
          context.addIssue({
            ...
            message: "Testimonials sections only accept testimonial blocks",
          });
        }
        addMaxTextIssue(
          context,
          settingText(block.settings, "quote"),
          1_000,
          ["blocks", index, "settings", "quote"],
          "Testimonial quote",
        );
        addMaxTextIssue(
          context,
          settingText(block.settings, "name"),
          120,
          ["blocks", index, "settings", "name"],
          "Testimonial name",
        );
        addMaxTextIssue(
          context,
          settingText(block.settings, "role"),
          160,
          ["blocks", index, "settings", "role"],
          "Testimonial role",
        );
      });
    }
      ...
      section.blocks.forEach((block, index) => {
        if (block.type !== "faq") {
          context.addIssue({
            code: "custom",
            path: ["blocks", index, "type"],
            message: "FAQ sections only accept FAQ blocks",
          });
        }
        addMaxTextIssue(
          context,
          ...
          240,
          ["blocks", index, "settings", "question"],
          "FAQ question",
        );
        addMaxTextIssue(
          context,
          settingText(block.settings, "answer"),
          1_600,
          ["blocks", index, "settings", "answer"],
          "FAQ answer",
        );
      });
    }
...
export const storefrontCompositionSchema = z
  .object({
  ...
  .superRefine((composition, context) => {
    ...
    if (new Set(ids).size !== ids.length) {
      context.addIssue({
        code: "custom",
        message: "Section and block ids must be unique",
      });
    }
  });

const storefrontContactSchema = z
  .object({
    phone: text(64),
    whatsapp: text(64),
    email: text(254),
...
const storefrontBuilderSchema = z
  .object({
  ...
  .superRefine((builder, context) => {
    const keys = builder.shippingRules.map(
      (rule) => `${rule.wilayaCode}:${rule.deliveryMode}`,
    );
    if (new Set(keys).size !== keys.length) {
      context.addIssue({
        code: "custom",
        path: ["shippingRules"],
        message: "Duplicate shipping rule",
      });
    }
  });

export const storefrontStudioThemeSchema = z
  .object({
...
export const storefrontStudioDraftSchema = z
  .object({
