...
export interface CommAgentConfig {
  enabled: boolean;
  auto_extract: boolean; // Automatically extract orders from messages
  suggest_replies: boolean; // Generate reply suggestions
  auto_send: false; // NEVER auto-send (draft-only for V1)
        ...
        try {
          ...
          if (seller?.shipping_rates) {
            ...
            const wilaya = WILAYAS.find(
              (w) =>
                w.name.toLowerCase() ===
                (extractedOrder.wilaya || "").toLowerCase(),
            );
