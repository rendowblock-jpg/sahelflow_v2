...
2. Read FD-028 through FD-031 in
...
4. Read the active phase and mandatory pre-Phase-8 gate in
...
9. For frontend adoption work, read
...
- Protected `main`: `04adb20fb5846499039eda61a9b765deb9c622e6` — PR #236.
...
- Shared frontend foundation is source/browser protected through PR #236.
- The only active implementation package is **PR #237 — Inbox operational
  workspace redesign**, branch `agent/inbox-product-workspace-redesign`, exact
  handoff head `cf84491cfd7613728a86dc9157da3fc4631e9105`.
- PR #237 is open/mergeable but **red and unmerged**; resume it rather than opening
  a competing implementation package.
...
Internal.14 installed use changed the execution frontier. The Founder explicitly
values the backend/engine and rejects the published frontend as the product-quality
baseline. Do not ask the Founder to enumerate every remaining pixel-level defect;
...
`876b0acdd2528df52ec106c22f231edf0b590739` retired the historical PR
#200 installed-UI waiver and PR #207 Phase 4 closure override as live evidence
bypasses and added anti-bypass regression authority.

### PR #233 — license activation continuity: CLOSED

`b91fd2a9008f529a5df3000d99bf426094f9daa9` repairs successful
permanent/trial activation so the server-authorized dashboard tree refreshes
without close/reopen.

### PR #234 — resilient customer trial source: PROTECTED; live certification OPEN

`bbfdc92e7b1845cd7cc4e2fd04c7ae5a2c7ab647` protects bounded
primary/recovery trial ingress, authoritative signed response selection,
privacy-safe diagnostics, owned-host release constraints and Worker readiness.
Issue #230 remains open P1 because owned production DNS, sufficiently independent
recovery routing, protected production bindings, representative Algerian networks
and signed installed customer evidence remain external to source/CI.

Exact #234 installed evidence also satisfied historical #201 hydrated-WebView and
#214 replacement-install obligations; both are closed. Reopen only from a concrete
new contradictory defect/evidence.

### PR #236 — shared frontend foundation: PROTECTED IN SOURCE/BROWSER

`04adb20fb5846499039eda61a9b765deb9c622e6` protects application Arabic
typography, atomic locale/direction transitions, one theme/density authority,
shallow navigation, governed notices/charts/motion, mixed-direction primitives,
resilient preference storage and independent coarse-pointer target sizing.

Frozen head `7d0b01a9f1989ad7e2cae25c3b0d39d6e92a64d8` passed CI
`31497523385`, Phase 5 `31497523052`, Phase 6–7 `31497523030` and final fresh
Codex review with zero unresolved material threads. This does **not** equal
installed Founder acceptance.

## Exact next outcome — continue PR #237 Inbox

Do not open another implementation branch while live PR #237 remains the active
package. Re-fetch the PR head/actions first; the exact handoff head is
`cf84491cfd7613728a86dc9157da3fc4631e9105` only until GitHub proves otherwise.

PR #237 changes Inbox product structure while preserving protected Phase 1–4 and
Phase 3 provider/inbox authorities:

- shop database remains visible history/workflow authority regardless of WhatsApp
  transport status;
- provider connected/reconnecting/disconnected/unavailable/QR remains separate
  transport health rather than silently switching the UI into demo mode;
- desktop becomes a task-shaped queue + durable thread/composer + workflow/team
  context workspace;
- All/Unread/Open/Pending/Resolved queues expose attention state directly;
- QR pairing and inbound recovery become bounded operational surfaces;
- durable send/outbox/retry/read receipts, message extraction, assignment/version/
  idempotency, collaboration and permission authority remain intact;
- AR/FR/EN and flow-relative RTL inherit the #236 foundation.

### Exact handoff repair batch for PR #237

Current exact-head gates are red. Working Memory owns full run/job details. At
minimum, the next session must batch-fix:

1. three ESLint `react-hooks/set-state-in-effect` errors in
   `inbox-workspace.tsx`, `whatsapp-ingress-recovery-dock.tsx` and
   `use-inbox-workspace.ts` without disabling the rule;
2. Phase 5 Inbox evidence that asserted the offline composer note before selecting
   a conversation;
3. Phase 6 static completion failure from hard-coded user copy
   `: Enter · Shift+Enter`;
4. Phase 7 controlled-browser route p95 over the 8s clean-CI tripwire (8.3s first
   attempt, 9.514s retry). Diagnose evidence/product cause; do not simply loosen
   the threshold without proof.

Complete a coherent Inbox self-audit before rerunning all selected gates. Once a
single exact head is green, request **one** fresh adversarial Codex review, batch
any real findings, and merge only with expected-head protection.

After Inbox: **AI Agents → Settings → remaining route inventory**.
...
- risk-selected CI/evidence authority.

Avoid schema/migration/native changes in frontend workspace packages unless a
concrete defect makes them necessary and the consequence-selected evidence is
...
- adopt one SahelFlow-specific decision with measurable acceptance/evidence;
...
- Core authority WIP 1; seller vertical WIP 2; experience/Arabic WIP 1;
...
Use the audit-first sequence with the selected consequence-appropriate gates:
...
- **P0:** active data loss, secret exposure, cross-shop effect, corrupt update/
  restore or irreversible stock/money damage. Stop immediately.
- **P1:** required journey/authority failure, negative/double stock, incorrect
  money, duplicate/lost effect, unsafe migration, startup/install/recovery failure,
  or major unusable Arabic/UX/accessibility defect. Blocks ordinary merge/release.
...
- do not run source builds, full suites, coverage or dependency installation when
  Actions can prove them;
