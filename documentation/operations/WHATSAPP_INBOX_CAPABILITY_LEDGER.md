...
| Delivery/read receipts | implemented-unproven | One canonical dependency-free mapper (`sidecars/whatsapp/delivery-status.ts`, #350) pinned at runtime against the installed Baileys 6.17.16 proto enum {ERROR:0,PENDING:1,SERVER_ACK:2,DELIVERY_ACK:3,READ:4,PLAYED:5}; monotonic persisted status projection retained | `delivery-status.test.ts` 7/7 incl. the runtime installed-enum cross-check | Packaged in published Internal.29 WITHOUT the enum repair; the repair is unreleased on main | **The Internal.27 certification is undermined**: the legacy mapper rendered SERVER_ACK as delivered and DELIVERY_ACK as read, so the observed "delivery" was SERVER_ACK and any "read" observation was DELIVERY_ACK | Re-prove delivery AND read states on the installed successor with the truthful mapper (requires #350 packaged in a signed release) |
