...
- Draft PR #115, `agent/updater-release-contract`, records the ChatGPT lead updater/release trust contract and fail-closed source guardrails from current `main`. It keeps updater activation, signing and publication disabled; no executable, artifact or installed-Windows evidence is claimed from the connector-only pass.

Exact next executable action — use this sequence in both new ChatGPT and Codex Cloud sessions:

1. **ChatGPT lead pass — proposed in draft PR #115:** the updater/release/versioning/migration/signing boundaries, governing implementation contract and activation guardrails are recorded without authorizing a key, channel or release. Executable validation and independent review remain required before merge or activation.
2. **Codex Cloud bootstrap:** declare Codex Cloud Linux, repository/branch/commit/worktree; prove dependency setup, development-app launch and the appropriate shared gate from current `main` without making an installed-Windows claim. Then check out `agent/updater-release-contract`, run `bun run sf-version` and `bun run sf-verify --fast`, and continue the coherent updater outcome PR.
3. **Updater implementation:** continue on `agent/updater-release-contract`; enable signed updater artifacts and checking only after the approved key/channel decisions; implement the complete checking/available/downloading/verifying/installing/restart/later/failure/retry journey; synchronize versions; create the GitHub Actions publishing path and signed update metadata; add tests and evidence. Do not place private signing material in the repository or prompts.
...
7. **A→B proof:** publish a deliberately small version B through the same signed channel; from the installed version A, check, download, verify, install and restart into B. Prove that shops, databases, settings, credentials, migration history and recovery material remain intact, and exercise offline/download/signature/install/restart failure handling.
