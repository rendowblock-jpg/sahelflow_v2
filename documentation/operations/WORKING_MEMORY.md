...
- App `1.0.0-internal.35`; MSI `1.0.0.35`; authority **FD-058**; mode `founder-offline-only` (published 2026-09-06; the Founder's installed checkpoint remains Internal.34 until the next in-place update — see the top Current-truth bullet).
...
source and **merged to protected `main`** (PR #391, squash `1847396…`), with
the INB-27 god-hook split merged on top (PR #392, squash `9dc8e74…`). No
open implementation PR remains. Remaining order:
...
1. Re-resolve protected `main`, open PRs and #306/#316/#317/#230. The
   ...
   (INB-13/14/19/32) stay BLOCKED. The UI/UX triage ledger has zero open
   source rows after the INB-27 merge.
2. Only an explicit Founder release directive packages the #391+#392 lines
   into the next signed successor (release train per `WORKFLOW.md`):
   release-authority PR, exact-head review, Required battery, expected-head
   merge, signed dispatcher/publication/observer, then one preserved
   in-place update. No release authority exists until that directive.
