...
The Founder updated to the signed Internal.16 build and explicitly confirmed that these original Internal.15 problem classes remain materially unresolved:
...
The Founder is not required to enumerate every sibling manifestation. The next writer owns complete source/component/route discovery for each established class. Absence of a new complaint about any other Internal.15 finding is not acceptance evidence.
...
4. **Run one focused root audit before coding** for the six reconfirmed classes:
...
5. **Freeze one bounded correction map** that names shared roots, affected production consumers, direct tests, localization/accessibility/performance consequences and non-goals. Do not create another permanent masterplan.
...
7. **Use targeted cheap checks during implementation.** Do not repeatedly build MSI after tiny edits and do not retry-away deterministic failures.
8. **Freeze one correction head** only after the six classes and their discovered siblings are source-complete.
9. **Run deep exact-head proof:** authority/current-frontier checks, TypeScript, ESLint, full relevant Vitest, complete Phase 5/6 browser journeys, AR/FR/EN + RTL/reflow/accessibility matrices, theme/motion/chart matrices, Inbox/AI matrices, Windows/native/install/authenticated UI evidence and consequence-selected performance checks.
10. **Repair one consolidated finding set**, then rerun affected + final proof.
11. **Only after the source candidate is genuinely clean**, request/record exact Internal.17 release authority, publish a signed candidate, install it on the Founder T470 and rerun #221 acceptance.
12. After #221 passes, complete #226 performance/reliability and #230 customer-trial/network evidence before user-ready/Stable claims.

## Acceptance rules for the six blockers
...
- Do not claim the six blockers fixed from unit/browser evidence alone; Founder installed acceptance remains required.
- Do not infer acceptance for unmentioned Internal.15 findings.
