...
PR #262 / `agent/internal19-product-convergence` is merged historical source convergence. PR #263 is governance-only reconciliation. Latest package authority remains Internal.18 / `1.0.0-internal.18` / MSI `1.0.0.18` / FD-037 / `founder-offline-only`. **No Internal.19 release authority exists yet.**
...
The fixed Internal.19 **application/source baseline** is `8448c47123290f2e1af702ff24a427cc11c4781c` from PR #262. Later governance-only docs/verifier merges may advance `main` without changing that application baseline.
...
FD-037 is historical/executed Internal.18 release authority only. The Internal.19 source merge and governance-only reconciliation are **not** release authority. A newer explicit Founder decision is required before a new signed package/version is produced.

## Exact next actions

1. Re-fetch live protected `main` and use that exact SHA as the parent for any new branch; never infer it from this document.
2. Do not modify package/version/release request/native/signing allowlists until a newer explicit Founder release decision exists.
3. If a next Founder checkpoint is explicitly authorized, create one bounded release-authority/version follow-up from the then-live protected main.
4. Run the release-relevant source/native/Windows/MSI/signing gates on that exact new authority head; do not reuse PR #262 evidence across a material package/version change.
5. Publish only exact protected-main signed artifacts authorized by the new decision.
...
8. Do not claim Beta/Stable/customer-online readiness from source merge or Founder-only package evidence.

## Hard rules

- one active application writer;
- retry-free required browser evidence;
...
Resolve live protected `main` from GitHub, preserve application baseline `8448c47123290f2e1af702ff24a427cc11c4781c` as historical/source identity, and continue with the **Exact next actions** above. Do not restart historical phases or publish a new Internal version until explicit newer Founder release authority exists.
