# SahelFlow Ultimate Design System (v2.1)
...
| 3 | **Free Tier Only** | Every seller runs on their own free-tier AI account (Gemini 3.5 Flash). All processing happens locally on the seller's device. No VPS, no server costs. SahelFlow costs $0/month to run, at any scale, forever. | Costs that scale with clients; surprise bills; vendor lock-in |
...
| **All third-party credentials** | OS keychain (Windows Credential Manager / macOS Keychain / Linux Secret Service) | AI keys, delivery provider tokens (Yalidine/ZR Express/Maystro), e-commerce integration tokens — ALL stored in OS keychain, never in SQLite. One secure store, encrypted at rest by the OS, survives even if SQLCipher key leaks. Consistent with AI-key decision. |
...
|---|------|-----------|--------|------------|
| 1 | **Gemini rate limits hit (per-client)** | Medium | Medium | Seller uses own account. Regex fallback handles ~70% of messages. Power users (>50 chat questions/day) may approach limit — add Groq as power-user upgrade in Phase 2 if needed. |
...
| **v2.2** | **2026-06-21** | **Drift fixes + credential storage decision locked.** (1) Title corrected v2.0 → v2.1. (2) Principle 3: removed stale Groq reference (Gemini-only per v2.1). (3) Risk #1: Groq/Gemini → Gemini; mitigation updated to reflect regex fallback + Phase 2 Groq upgrade path. (4) Success metrics: trial-to-paid >20% → >30% (matching v2.1 changelog). (5) NEW locked decision: all third-party credentials (AI keys + delivery tokens + e-commerce tokens) stored in OS keychain, never in SQLite — consistent with AI-key decision, protects against SQLCipher key leakage. |
...
_Last updated: 2026-06-21 — v2.2. Drift fixes (title, Principle 3, Risk #1, metrics) + credential storage decision locked (OS keychain for all secrets). v2.1: Gemini-only + regex fallback. v2.0: Full redesign. Local-first Tauri architecture. No hard deadlines. Ship when ready._
