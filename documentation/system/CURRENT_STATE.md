...
> **Live protected main:** resolve from GitHub before every action; best-known local `origin/main` is `9f1a58a…` after PR #426 (STR-01 slice 1 — the MessageBubble cluster extracted into `inbox-thread-message.tsx`, 21 source pins re-anchored, behavioural suite green) on top of #425 (canonical money/phone formatters; the built-but-unreachable `change-pin` and shop-lifecycle authority wired into Settings with route-auth/reachability contracts), #424 (Internal.37 Founder visual repair F-14 — inbox queue/header density + RTL names, canvas-first Agents presentation), #423 (release authority: FD-060 adopted — Internal.37 signed successor packaged the #413–#422 frontier; one combined FD-058 campaign on the installed Internal.37, skipping Internal.35/.36), #422/#421/#420/#419/#418/#416/#415/#414 (FD-060 packaged transformation + STR-01 canvas slices), #413 (Internal.36 publication record), #412 (Internal.36 release authority), #411/#409/#408/#406/#405/#403/#402/#400 and the #399–#410 docs line; re-resolve live before acting
> **Current signed release:** Internal.37 / `1.0.0-internal.37` / MSI `1.0.0.37` / FD-060 (release authority PR #423, squash `cd1114de…`; tag `sahelflow-v1.0.0-internal.37-cd1114defc59852a8f4a2258147dbe57b7fa4050`; the updater `latest.json` published 2026-09-11T12:56:24Z serves signed `windows-x86_64` + `windows-x86_64-msi` Internal.37 manifests; the Founder installed Internal.37 in place 2026-09-11 and visually rejected it before campaign conversion (F-14/F-15, repaired at source by #424); certification facts for the exact release head are retained in the FD-060 decision record — quote them from `../product/DECISIONS.md`, never restate from memory)
> **Latest Founder-installed checkpoint:** Internal.37 / FD-060 (installed in place 2026-09-11, AppData preserved; `runtime-ui-ready.json` `1.0.0-internal.37`, locale `ar`) — visually rejected before any campaign-row conversion: F-14 inbox queue density + RTL name placement, F-15 Agents workspace presentation; both repaired at source by merged PR #424 (F-14 inbox + F-15 canvas-first Agents on the existing engine), which converts no installed row. Historical: Internal.34 / FD-057 was installed 2026-09-05; Internal.35 (`f45e6e1…`, FD-058) and Internal.36 (`4e527f05…`, FD-059) were never Founder-installed and remain retained publication evidence. Observing the merged #424–#427 line on an installed candidate requires the Founder's packaging decision
> **Current execution:** the release freeze was lifted by the Founder directive 2026-09-02 ("complete all the work professionally and flawlessly then make the update") — the consolidated UI/UX triage line was executed on PR #387 (AAA message-surface waves F-01..F-03, ledger completion waves, INB-24 voice gestures, AI-21 screenshot extraction, INB-16 link previews, AI-26 truthful turn signals, INB-28 sender factory, INB-11 virtualization, AI-13 feedback loop, INB-12 pin/mute/archive, qs 6.16.0 security pin, archive-queue type truth), certified at product head `92c35bc…` (tree-identical to the #387 squash; 20 success / 3 risk-classified skips / 0 failed incl. installed-MSI evidence + gates) and packaged/published as Internal.33 under FD-056 (dispatcher `33704527056`, signed publication `33704536249`, observer `33704547389` — all success). Next: **PR #391 was merged** to protected `main` by expected-head squash `1847396…`, repairing all five installed findings: F-04 delete-contract id space (64→256), F-05 thinking-model chat budget (2048→8192) + truthful empty-shape verdicts, F-06 agents-page functional completion executed after the Founder's scope correction (functional, NOT colors/motion/CSS — capability truth via `GET /api/ai/capabilities`, Abilities workforce panel, honest 5-count shop briefing, shop-wide approval loop, presentation-only shop context), F-07/F-08 queue header row ownership + shrink-proof select toolbar. In the same session the Founder authorized and the agent executed **INB-27** (the deliberately deferred god-hook split) at source on the stacked branch `refactor/inb-27-inbox-god-hook-split` — `src/hooks/inbox/` concern hooks (chat queue, drafts, thread, outbox, transport, shared refs + shared pure layer) composed by the unchanged `use-inbox-workspace.ts` facade; 10 source-pin contract files re-anchored with identical invariants; targeted 203/203 in a fresh sandbox, ESLint 0 errors; the first hosted CI battery on the rebased head then caught a single rebase-replay defect (missing `useRef` import in the facade — `TS2552` plus an inbox-page runtime crash; the local full-project tsc claim was unreliable at sandbox memory limits), fixed at `af99798`. **PR #392 was then MERGED** to protected `main` by expected-head squash `9dc8e74…` under the Founder's directive 2026-09-05 (live re-resolution before the write: base `1847396…` equal to main, head `80395e2c…`, mergeable clean, hosted Required battery at the exact head 14 success / 5 risk-classified skips / 0 failed; the merge call pinned the expected head SHA; branch deleted; durable tree verified). The UI/UX triage ledger now has ZERO open source rows. Only an explicit Founder release directive packages the #391+#392 lines into the next signed successor; rows convert on the next installed observation, the wave rows + D3 six waves re-verify, the #306 logout row stays LAST, and the screenshot-exposed Gemini key (#373) is rotated before real AI usage. Sidecar rows (INB-13/14/19/32) stay BLOCKED. **That directive arrived and was executed the same day (2026-09-05): the Founder's "Release Internal.34" authorized FD-057 — release PR #394 merged by expected-head squash `0cdd5ce…` after its head `2ac661e…` passed the FULL Required battery (19 checks, 0 failed, incl. installed-MSI evidence + lifecycle gates); certification cited product head `9dc8e74…` (tree-identical to PR #392 head `80395e2…`; CI `33961402161`, Phase 5 `33961402071`, Phase 6-7 `33961402075`); dispatcher `33966937514`, signed publication `33966945628` and observer `33966952222` all succeeded; release tag `sahelflow-v1.0.0-internal.34-0cdd5ce…`; published MSI digest `sha256:dc3d3771…`; updater `latest.json` pub_date 2026-09-05T13:04:24Z serves Internal.34 to the installed Internal.33. The Founder then INSTALLED Internal.34 in place and ran its campaign (2026-09-05): F-04 partially converted (the 256 bound works; a first-attempt intermittent delete error re-rooted as SQLite write-lock contention → F-10), F-05 not converted as observed (the client collapsed every SSE error into a false "provider unavailable" title → F-09; the founder then rotated the Gemini key, meeting the #373 precondition), F-06 re-rejected with the directive "top tier class AAA redesign" → F-12, F-08 not converted (count-pill collapse → F-11), F-07 stayed green, voice notes + document cards stayed green (B3/B4). All four roots + the directive were repaired at source in PR #396 (`fix: Internal.34 installed-campaign repairs` — F-09 truthful coded error surfacing + thought-token usage proof with the client rendering the server's locale-native verdict verbatim; F-10 bounded busy-retry on the deletion transaction; F-11 wrapping no-overlap select toolbar; F-12 Class-AAA presentation overhaul of the agents workspace with the engine untouched; 162 targeted tests green across delete/agent/inbox/AI/motion suites). Rows convert on the next installed observation only; #306 logout stays LAST.** **The Founder's 2026-09-06 directive ("go with 1 then 2" — merge both campaign PRs, then cut the signed successor) was executed the same day: PR #396 merged by squash `dcd82f8…` (AI line: F-09 truthful coded verdicts, F-10 busy-retry delete, F-11 wrapping toolbar, F-12 Class-AAA agents, and the F-05 residual ROOT CAUSE — the Gemini `alt=sse` parser split events on LF-LF only while Google frames with CRLF, so every real chat turn parsed bare-empty while the non-streaming verify probe succeeded; repaired by per-chunk CR-stripping + `ai.gemini.stream_empty_turn` evidence log + CRLF fixtures; plus the CI-repair `6ceb79e` for the `noUncheckedIndexedAccess` delay index the F-10 commit introduced), PR #397 merged by squash `d83bd8e…` (F-13 notifications show-all workspace: N-1 pagination wipe, N-2 duplicate-id rendering, N-3 per-row mark-read, N-4 preferences skeleton, N-5 unread surfacing, N-6 preference-state chips, N-7 semantic day headers, N-8 inbox-v3 grammar) after the predicted UI_UX_TRIAGE_LEDGER adjacency conflict was resolved by merge `ccded6a…`; release PR #398 (FD-058) merged by squash `f45e6e1…` after its head passed the FULL release battery (20 success / 1 risk-classified skip / 0 failed incl. installed-MSI evidence + lifecycle gates); certification cited product head `d83bd8e…` (tree-identical to the reviewed PR #397 head `ccded6a…`, tree `a29d4d7b…`; CI `34016720713`, Phase 5 `34016720554`, Phase 6-7 `34016720602`); dispatcher `34019675230`, signed publication `34019683179` and observer `34019691336` all succeeded; release "SahelFlow Internal 1.0.0-internal.35" published 2026-09-06T08:03:22Z at tag `sahelflow-v1.0.0-internal.35-f45e6e1…`; published MSI digest `sha256:97bcd5dccd486e9a212dd04b37e22c3d29ccdeb9685e4ea1e6b0d6e845dca502`; updater `latest.json` pub_date 2026-09-06T07:59:43Z serves Internal.35 to the installed Internal.34. Next: the Founder installs Internal.35 in place through the normal updater and its campaign re-verifies the F-05 residual (chat must stream for the first time), F-09..F-13 and the retained rows; #306 logout stays LAST; the chat-transited GitHub PAT is rotated after the merge window.** Exact rows: `../operations/UI_UX_TRIAGE_LEDGER.md` + `../operations/INTERNAL_30_CAMPAIGN_RECONCILIATION_LEDGER.md`
...
Internal.35 is the latest signed/published artifact (header truth; §§1–2b below retain the Internal.30–Internal.29 history and remain valid as retained evidence — see `operations/WORKING_MEMORY.md` for the Internal.31–Internal.35 train):
