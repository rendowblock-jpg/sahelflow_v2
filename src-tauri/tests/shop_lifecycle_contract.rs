...
use crate::shop_lifecycle_command::{
    lifecycle_command_key, ShopLifecycleAuthorization, ShopLifecycleCommand, ShopLifecyclePayload,
};
...
fn rust_matches_the_node_lifecycle_command_key() {
    ...
    assert_eq!(
        key,
        [
            0x8d, 0x48, 0x79, 0xa0, 0xd5, 0x57, 0x20, 0x6d, 0x03, 0x9e, 0x4f, 0xd0, 0x83, 0x59,
            0x78, 0x86, 0x10, 0x8b, 0xbf, 0x1f, 0xc5, 0x42, 0x74, 0x87, 0xd4, 0x71, 0x7d, 0x13,
            0x64, 0x3b, 0xe4, 0x23,
        ]
    );
}
...
fn rust_accepts_the_node_golden_lifecycle_ticket() {
    assert_eq!(node_golden_command().verify(&[9_u8; 32], 1_001_000), Ok(()));
}
