...
fn mark_planned_generation_failed(
    app: &tauri::AppHandle,
    generation: u64,
) -> Result<(), IoError> {
    let state = app
...
fn show_manual_recovery(app: &tauri::AppHandle, detail: &str) {
    if let Err(error) = super::show_blocked(
        ...
        &format!(
            "Native shop switching could not prove safe compensation: {detail}. SahelFlow will not start another lifecycle operation until the installation is recovered."
        ),
    ) {
...
fn reject_claim(
    ...
    let rejected = app_data_dir
        .join(INBOX_DIRECTORY)
        .join(REJECTED_DIRECTORY);
